...
export abstract class BaseService<T> {
  ...
  async findOne(
    ...
    return await this.model.findOne(
      { ...filters, status: { $ne: E.StatusEnum.DELETED } } as FilterQuery<T>,
      projection,
      options,
    );
  }
  ...
  async findActive(
    ...
    return await this.model
      .find({ ...filters, status: E.StatusEnum.ACTIVE } as FilterQuery<T>)
      .sort({ ...sort })
      .exec();
  }
  ...
  async getPaginatedList(
    ...
    newFilters.status = { $ne: E.StatusEnum.DELETED };
    if (status) {
